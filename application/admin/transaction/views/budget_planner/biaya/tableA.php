...
foreach ($coa as $k => $v) {
						...
						foreach ($detail['3'][$v3->glwnco] as $k4 => $v4) {
							...
							if(isset($detail['4'][$v4->glwnco])){
								foreach ($detail['4'][$v4->glwnco] as $k5 => $v5) {
									...
									$item5 .= '<td><div style="min-height: 10px; width: 100%; overflow: hidden;" contenteditable="false" class="edit-value text-right edited" data-name="biaya_bulan" data-id="'.$v6->glwnco.'" data-value="'.$valueTxt.'">'.$valueTxt.'</div></td>';
							...
							$item4 .= '<td><div style="min-height: 10px; width: 100%; overflow: hidden;" contenteditable="false" class="edit-value text-right edited" data-name="biaya_tahun" data-id="'.$v4->glwnco.'" data-value="'.$bln_trakhir.'">'.$bln_trakhir.'</div></td>';
							$item4 .= '<td'.$bd.'></td>';
	...
	for ($i=1; $i <= 12 ; $i++) { 
		...
		$dataSaved['bulan_b'.$i] = $val;
		$item .= '<td '.$tr.'><div style="min-height: 10px; width: 100%; overflow: hidden;" s contenteditable="false" class="edit-value text-right edited" data-name="bulan_b'.$i.'" data-id="'.$v->glwnco.'" data-value="'.$val.'">'.check_data($val,$minus).'</div></td>';
		$biaya_tahun1 += $val;
	}
	checkForUpdate($dataSaved);
	$item .= '<td'.$bd.'></td>';
